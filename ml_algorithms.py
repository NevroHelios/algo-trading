        ...
        try:
            if current_index < self.lookback_period:
                return {
                    "signal": "HOLD",
                    "strength": 0.0,
                    "reason": "Insufficient data for PCA",
                }

            # Get recent window of data
            window_data = df.iloc[
                max(0, current_index - self.lookback_period) : current_index + 1
            ]
            ...
            if transformed_data is None:
                return {
                    "signal": "HOLD",
                    "strength": 0.0,
                    "reason": "PCA transformation failed",
                }
            ...
            explained_variance = (
                self.pca.explained_variance_ratio_[0] if self.pca else 0
            )

            # Generate signal based on trend and momentum
            signal_strength = min(
                float(abs(recent_trend) * explained_variance * 2), 1.0
            )
        ...
        try:
            if current_index < self.min_observations:
                return {
                    "signal": "HOLD",
                    "strength": 0.0,
                    "reason": "Insufficient data for ARIMA",
                }
        ...
        try:
            if current_index < self.min_observations:
                return {
                    "signal": "HOLD",
                    "strength": 0.0,
                    "reason": "Insufficient data for GARCH",
                }
            ...
            return {"signal": signal, "strength": signal_strength, "reason": reason}

        except Exception as e:
            return {
                "signal": "HOLD",
                "strength": 0.0,
                "reason": f"GARCH error: {str(e)[:50]}",
            }
...
class MLEnsemble:
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        self.combination_method = self.ensemble_config.get(
            "combination_method", "weighted_average"
        )
        self.minimum_algorithms = self.ensemble_config.get("minimum_algorithms", 2)
        self.confidence_threshold = self.ensemble_config.get(
    ...
    def generate_ensemble_signal(
        ...
        if not self.algorithms:
            return {
                "signal": "HOLD",
                "strength": 0.0,
                "reason": "No ML algorithms enabled",
            }
        ...
        valid_signals = {
            k: v
            for k, v in algorithm_signals.items()
