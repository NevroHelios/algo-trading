...
class Trade:
    ...
    def __init__(self, action: str, quantity: int, price: float, timestamp: datetime, fees: float = 0.0):
        ...
        self.net_amount = self.gross_amount + fees if action == 'BUY' else self.gross_amount - fees
...
class EnhancedPortfolio:
    ...
    def calculate_trading_costs(self, action: str, quantity: int, price: float) -> float:
    ...
    def calculate_effective_price(self, action: str, market_price: float) -> float:
        ...
        if action == 'BUY':
            ...
            effective_price = market_price * (1 + self.bid_ask_spread/2 + self.slippage/2)
        else:  # SELL
    ...
    def _execute_buy(self, market_price: float, timestamp: datetime) -> bool:
        ...
        if self.cash >= total_cost:
            ...
            # Create tax lot
            cost_basis_per_share = (effective_price * quantity + actual_costs) / quantity
    ...
    def _execute_sell(self, market_price: float, timestamp: datetime) -> bool:
        ...
        self._calculate_realized_gains(quantity, effective_price, timestamp, trading_costs)
        
        return True
    
    def _calculate_realized_gains(self, quantity_sold: int, sale_price: float, 
                                sale_date: datetime, sale_costs: float):
    ...
    def get_current_value(self, current_price: float) -> float:
        ...
        if self.position > 0:
            current_cost_basis = sum(lot.quantity * lot.cost_basis for lot in self.tax_lots)
    ...
    def summary(self):
        ...
        print(f"Total return (after tax): ${after_tax_return:,.2f} ({after_tax_return_pct:+.2f}%)")
        ...
        print(f"\n📈 TRADING STATISTICS:")
        print(f"Total trades: {len(self.trades)} (Buy: {self.buy_count}, Sell: {self.sell_count})")
        ...
        if self.realized_gains != 0:
            tax_efficiency = (1 - self.total_taxes_paid / abs(self.realized_gains)) * 100
        ...
        if self.tax_lots:
            ...
            for i, lot in enumerate(self.tax_lots):
                ...
                status = "Long-term" if days_held >= self.holding_period_days else "Short-term"
                print(f"  Lot {i+1}: {lot.quantity} shares @ ${lot.cost_basis:.2f} ({days_held} days, {status})")
